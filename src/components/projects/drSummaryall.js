...
const NoticeSummary = ({ project}) => {
  ...
  return (
    <div className='card' >
      ...
      <span className="black-text" >Dr. <b>{project.name}</b>({project.sp}) <span><i>-{project.visitHr}</i></span> </span>
     <span className='hide'>
     ...
     {project.visitday[6]?<span>{project.visitday[6].toUpperCase()}</span>:null}</span>
